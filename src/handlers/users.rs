...
pub fn create_user(req: &mut Request, res: &mut Response) -> String {
    ...
    let user_data: db::User = {
        serde_json::de::from_reader(&mut req.origin).unwrap()
    };

    // hash the password
    // TODO FIXME XXX An application that requires an entropy source for
    // cryptographic purposes must use OsRng
    ...
    match rows {
        Ok(rows) => {
            ...
            let user_id = db::ReturnId {
                id: first_and_only_row.get("id"),
            };

            serde_json::ser::to_string(&user_id).unwrap()
        },
